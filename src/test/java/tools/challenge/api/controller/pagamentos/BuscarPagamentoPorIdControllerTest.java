...
@QuarkusTest
public class BuscarPagamentoPorIdControllerTest {
    ...
    void deveSerPossivelBuscarUmPagamentoPorIdDaTransacao() {
        ...
        responseDeCriacaoDePagamento(TransacaoInput.from(transacao));
        final Transacao transacaoPersistido = repository.findAll().getFirst();
        final String path = "/pagamentos/" + transacaoPersistido.id().toString();
        ...
        assertEquals(TransacaoStatus.AUTORIZADO.toString(), output.descricaoOperacao().status());
    ...
    void deveSerPossivelBuscarTodosPagamentosVazio() {
        final String path = "/pagamentos/" + TransacaoID.of();
